...
(define-syntax-rule (localvar-behavior (varid varcount) getter value setter)
  (begin (reduction-simple (allocate-argument-space (a-count integer?))
                           (boxdag/export argument-counts count a-count))
         (reduction-calc (local (varid any?)) (cur-exports)
                         (let ((varcount (second (assoc 'count (second (assoc 'argument-counts cur-exports))))))

...
;(define (find-placeholder tree)
;  (cond ((placeholder? tree)
;         (if (number? (placeholder-get tree))
;             null
;             tree))
;        ((pair? tree)
;         (let ((a (find-placeholder (car tree))))
;           (if (not (null? a))
;               a
;               (find-placeholder (cdr tree)))))
;        (else null)))
;
;(define (process-placeholder next-id placeholder)
;  (let ((block (placeholder-get placeholder)))
;    (assert (pair? block) "Expected pair in placeholder.")
;    (placeholder-set placeholder next-id)
;    (blockify block)))
;
;(define (blockify next-id tree)
;  (let ((placeholder (find-placeholder tree)))
;    (if (null? placeholder)
;        (list tree)
;        (let ((processed (process-placeholder next-id placeholder)))
;          (append
;           processed
;           (blockify (+ next-id (length processed)) tree))))))
